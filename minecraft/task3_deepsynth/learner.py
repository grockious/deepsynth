...
if __name__ == '__main__':
    ...
    sar = mine_craft_env.exploration(3, 500, 1000)
    sar_1 = np.array(sar[1])
    sar_2 = np.array(sar[2])
    sar_3 = np.array(sar[3])
    sar_4 = np.array(sar[4])
    ...
    sar_4 = np.delete(sar_4, random.sample(range(0, len(sar_4)), len(sar_4) - exp_size + len(indx_4[1])), axis=0)
