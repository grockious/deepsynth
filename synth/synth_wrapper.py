...
def dfa_update(trace, num_states, dfa_model, nfa_model, model_gen, var, input_dict, hyperparams, start_time, iter_num):
	...
	for event_list in events_tup_to_list:
		...
		model_gen, var, input_dict, num_states = make_model(event_list, model_gen, var, hyperparams, num_states, input_dict, start_time)

	nfa_model = model_gen.copy()
	dfa_model, var, input_dict, num_states = convert_to_dfa_plot(full_events, model_gen, input_dict, num_states, hyperparams, var, start_time, iter_num)
	...
	try:
		...
		for e_id in temp:
			...
			if len(next_state) > 1:
				...
				exit(0)
			if not next_state:
				print(colored("[WARNING] No next state found for label " + event_uniq[e_id-1],'magenta'))
				return []
			state = next_state[0]
...
# for trace in traces:
# 	num_states, processed_dfa, dfa_model, nfa_model, model_gen, var, input_dict = dfa_update(trace, num_states, dfa_model, nfa_model, model_gen, var, input_dict, hyperparams, start_time, iter_num)
# 	iter_num = iter_num + 1

# print("next_state")
# next_state_dfa = get_next_state(traces[0][:5], input_dict['event_uniq'], processed_dfa)
# print(next_state_dfa)
